...
const RightSidebar = () => {
  ...
  useEffect(() => {
    fetchNotifications();
    fetchTrendingTags();
    const interval = setInterval(fetchNotifications, 30000); // auto-refresh notifications every 30s
  ...
  return (
    <aside className="hidden lg:flex flex-col gap-8 w-80 shrink-0 pt-8 px-6 bg-[#181920] border-l border-[#2CB67D]/10 min-h-screen">
      {/* Notifications */}
      <section>
        <h3 className="text-lg font-bold text-[#2CB67D] mb-3">Notifications</h3>
        <div className="flex flex-col divide-y divide-[#2CB67D]/30 max-h-96 overflow-y-auto rounded-lg bg-[#22243e] p-2">
          {loading ? (
